...
- `scripts/make_chaos_figures.py`: helper used by the unified paper driver for
  transformed-theta and LIF covariance and stability figures.
...
The chaos and first-passage calculations are figure groups in the same paper:

```bash
python scripts/make_paper_figures.py \
  --profile paper \
  --figures chaos first-passage \
  --plot-dir /path/to/randomnet-results \
  --jobs 10
```

The helper scripts remain directly callable for focused development, but a
complete `make_paper_figures.py --figures all` run now includes their output.
...
The unified manuscript additionally uses the transformed-theta activity,
covariance, Lyapunov, and replica figures, the LIF summary, and the
first-passage/return-operator benchmark generated by the `chaos` and
`first-passage` groups.
